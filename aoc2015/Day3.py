def solve(directions):
    ...
    for direction in directions:
        ...
        ledger[location] = ledger.get(location, 0) + 1

    print(len(ledger))


def solve_p2(directions):
    locations = [(0, 0), (0, 0)]
    ledgers = [{locations[0]: 1}, {locations[1]: 1}]

    turn = 0
    for direction in directions:
        if direction == '^':
            locations[turn] = locations[turn][0] - 1, locations[turn][1]
        elif direction == '>':
            locations[turn] = locations[turn][0], locations[turn][1] + 1
        elif direction == 'v':
            locations[turn] = locations[turn][0] + 1, locations[turn][1]
        elif direction == '<':
            locations[turn] = locations[turn][0], locations[turn][1] - 1
            ...
            raise Exception("Invalid direction " + direction)
        ledgers[turn][locations[turn]] = ledgers[turn].get(locations[turn], 0) + 1
        turn = (turn + 1) % 2

    print(len(ledgers[0] | ledgers[1]))
...
solve(list(tst_input))
solve(list(input))

solve_p2(list(tst_input))
solve_p2(list(input))
